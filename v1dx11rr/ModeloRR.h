...
private:
	...
	ID3D11Buffer* diffuseCB;
	ID3D11Buffer* changeColorCB;

	ID3D11Buffer* movimientoCB;
...
public:
	float colorChange = 1;
	ModeloRR(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, char* ModelPath, WCHAR* colorTexturePath, WCHAR* specularTexturePath, float _posX, float _posZ, bool esaguaP = false)
	{

		m_color = XMFLOAT3(0.2, 0.2, 0.2);
		esagua = esaguaP;
		colorChange = 0;
	...
	bool CargaParametros(char* ModelPath, WCHAR* diffuseTex, WCHAR* specularTex)
	{
		...
		bool compileResult;
		if (!esagua) {
			compileResult = CompileD3DShader(L"Modelo.fx", "VS_Main", "vs_4_0", &vsBuffer);
		}
		else {
			compileResult = CompileD3DShader(L"Water.fx", "VS_Main", "vs_4_0", &vsBuffer);
		}
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &changeColorCB);
		//de vista
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &diffuseCB);

		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &movimientoCB);
	...
	bool UnloadContent()
	{
		...
		if (diffuseCB)
			diffuseCB->Release();
		if (changeColorCB)
			changeColorCB->Release();
		if (movimientoCB)
			movimientoCB->Release();
	...
	//	D3DXMATRIX worldMat = rotationMat * scaleMat * translationMat;
		...
		D3DXMATRIX worldMat;
		if (sigueCamara) {
			worldMat = scaleMat * sigueCamaraAjuste * rotationMat * translationMat;
		}
		else {
		...
		d3dContext->UpdateSubresource(diffuseCB, 0, 0, &diffuseCB, 0, 0);

		d3dContext->UpdateSubresource(changeColorCB, 0, 0, &colorChange, 0, 0);
		...
		d3dContext->VSSetConstantBuffers(5, 1, &diffuseCB);
		d3dContext->PSSetConstantBuffers(6, 1, &changeColorCB);


		if (esagua) {
			d3dContext->PSSetConstantBuffers(5, 1, &movimientoCB);
		}
